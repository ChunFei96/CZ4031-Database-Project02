...
if __name__ == '__main__':
    ...
    execute_button = tk.Button(root, text='EXECUTE QEP', padx=12, bg='#7d8ed1', fg='white', font=button_font,
                               anchor='center', command=lambda: interface.get_json(retrieveInput()))

    # button execution to execute function
    execute_button.bind('<Button-1>', lambda event: interface.execute_query(root, retrieveInput()))
    ...
    # drawing the execute button
    execute_button_AQP = tk.Button(root, text='EXECUTE AQP', padx=12, bg='#7d8ed1', fg='white', font=button_font,
                               anchor='center', command=lambda: interface.get_json(retrieveInput(), False))

    # button execution to execute function
    execute_button_AQP.bind('<Button-1>', lambda event: interface.execute_query_AQP(root, retrieveInput()))
    ...
    execute_button_AQP.grid(row=4, sticky='e', padx=200, pady=12)
