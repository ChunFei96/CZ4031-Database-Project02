...
def get_json(inputValue, QEP = True):
    ...
    try:
        ...
        cur = conn.cursor()
        if QEP is True:
            cur.execute(get_QEP() + inputValue)
        elif QEP is False:
            cur.execute(get_AQP() + inputValue)
        rows = cur.fetchall()
        x = json.dumps(rows)
...
def get_QEP():
    config = ""
    config += " EXPLAIN (ANALYZE, VERBOSE, FORMAT JSON) "
    return config

def get_AQP():
    config = ""
    config += " SET LOCAL enable_hashjoin = off; "
    config += " EXPLAIN (ANALYZE, VERBOSE, FORMAT JSON) "
    return config
...
class TreeFrame(tk.Frame):
    ...
    def _draw_node(self, node, tree, x1, y1):
        ...
        top = y1
        bottom = -1
        button = tk.Button(self.canvas, text=node.tag, padx=12,
        ...
        for child in children:
            child_bbox = self._draw_node(child, tree, child_x, y1 + 60)  # (x1, y1, x2, y2)
...
def execute_query(root_widget, query):
    ...
    treeQEP_highlight = Annotation().matchNodeToQuery(treeQEP, query)
    treeQEP_annotation = Annotation().annotate(treeQEP, query)
    tree_frame.draw_tree(treeQEP)

    def on_click_listener(node):
        if node.identifier in treeQEP_annotation:
            analysis_frame.show_node_annotation(treeQEP_annotation[node.identifier])

    def on_hover_listener(node):
        if node.identifier in treeQEP_highlight:
            for pos in treeQEP_highlight[node.identifier]:
                query_frame.highlight_text(pos[0], pos[1], node.tag)
...
def execute_query_AQP(root_widget, query):
    plan_AQP = get_json(query, False)
    ...
    treeAQP_highlight = annotation.matchNodeToQuery(treeAQP, query)
    treeAQP_annotation = annotation.annotate(treeAQP, query)
    tree_frame.draw_tree(treeAQP)

    def on_click_listener(node):
        if node.identifier in treeAQP_annotation:
            analysis_frame.show_node_annotation(treeAQP_annotation[node.identifier])

    def on_hover_listener(node):
        if node.identifier in treeAQP_highlight:
            for pos in treeAQP_highlight[node.identifier]:
                query_frame.highlight_text(pos[0], pos[1], node.tag)
